...
const ChatDesktop = () => {
  const handleSubmitChatDesktop = e => {
    e.preventDefault();
    ...
    Swal.fire({
      ...
      <strong>If Web, Provide Room URL:</strong> ${webURL}<br>
  ...
  return (
    <form onSubmit={handleSubmitChatDesktop}>
      ...
      ></input>
      <label className="labelForm" style={{ display: "block" }}>
        Direct Message (include username of the other party
        conversation):{" "}
      </label>
      <input
        className="inputForm"
